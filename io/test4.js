...
async function start(socket, sdpOffer) {
    ...
    const composite = waitingSocket ? waitingSocket.composite : await pipeline.create('Composite');
    socket.composite = composite;
    const hubPort = await composite.createHubPort();
    const endPoint = await pipeline.create('WebRtcEndpoint');
    socket.endPoint = endPoint;
    await endPoint.connect(hubPort);
    await hubPort.connect(endPoint);
